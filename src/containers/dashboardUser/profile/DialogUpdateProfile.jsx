...
const DialogUpdateProfile = props => {
    const { openDialogUpdate, handleToggleDialogUpdate, getUser, currentProfile = {}, setIsLoading } = props;
    ...
    useEffect(() => {
        if (openDialogUpdate) {
            const initialValues = {
                ...
                genderKey: currentProfile?.genderKey || '',
                CIC: currentProfile?.CIC || '',
                country: currentProfile?.country || '',
            };
            formik.setValues(initialValues);
            getAllGenders();

            setBirthday(dayjs(currentProfile?.birthday) || dayjs());
            setPreviewImage(currentProfile?.avatarData?.url || null);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentProfile, openDialogUpdate]);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting, resetForm }) => {
            try {
                ...
                const response = await updateUserService(axiosPrivate, currentProfile.id, formData);
                if (response) {
                    setIsLoading(false);
                    setSubmitting(false);
